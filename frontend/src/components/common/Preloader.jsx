...
const MatrixRain = ({ color }) => {
  ...
  useEffect(() => {
    ...
    const chars = '01XYZｦｧｨｩｪｫｬｭｮｯｰｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ';
    const fontSize = 14;
  ...
  return <canvas ref={canvasRef} className="absolute top-0 left-0 w-full h-full opacity-25" />;
};
        ...
        <div className="fixed inset-0 z-[9999] bg-black font-mono overflow-hidden cursor-none selection:bg-white selection:text-black">
          ...
          <div className="absolute inset-0 flex w-full h-full z-30 pointer-events-none">
            {[...Array(20)].map((_, i) => (
              <motion.div
                ...
                exit={{ 
                  scaleY: 0,
                  transition: { duration: 0.6, ease: "easeInOut", delay: i * 0.02 } 
                }}
              />
            ))}
          </div>
          ...
          <div className="relative z-40 flex items-center justify-center w-full h-full">
            <motion.div 
              ...
              exit={{ scale: 1.5, opacity: 0, filter: "blur(20px)", transition: { duration: 0.3 } }}
            ...
            >
              ...
              >
                {/* 1. Header Bar */}
                <div className="flex justify-between items-center mb-8 border-b pb-2" style={{ borderColor: `${themeColor}40` }}>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full animate-pulse" style={{ backgroundColor: themeColor }}></div>
                    <span className="text-xs tracking-[0.3em] font-bold" style={{ color: themeColor }}>
                       {isSuccess ? "SYSTEM READY" : "INITIALIZING SYSTEM"}
                    </span>
                  </div>
                  <span className="text-[10px] opacity-50" style={{ color: themeColor }}>V.2.0.4</span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="relative flex items-center justify-center">
                     ...
                     <div className="z-10 text-center">
                        <span className="text-6xl font-black tracking-tighter" style={{ color: themeColor }}>
                           {Math.floor(progress)}
                        </span>
                        <span className="text-xl ml-1" style={{ color: themeColor }}>%</span>
                     </div>
                  </div>
                     ...
                     <div className="flex flex-wrap gap-1 w-full opacity-50">
                        {[...Array(20)].map((_, i) => (
                           <div 
                              ...
                              style={{ backgroundColor: i < (progress / 5) ? themeColor : '#333' }}
                           />
                        ))}
                     </div>

                     {/* Scramble Logs */}
                     <div className="h-20 overflow-hidden text-xs md:text-sm font-light leading-relaxed" style={{ color: themeColor }}>
                        {bootLogs.slice(Math.max(0, logIndex - 4), logIndex + 1).map((log, i) => (
                        ...
                        <div className="text-center max-w-lg w-full p-4">
                           ...
                           {isGeneratingIdentity && (
                               <div className="text-green-400 font-mono animate-pulse">
                                   <p className="text-sm tracking-widest">✨ ESTABLISHING NEURAL LINK...</p>
                                   <p className="text-xs opacity-50 mt-2">DECRYPTING USER PROFILE VIA GEMINI_CORE...</p>
                               </div>
                           )}
                           ...
                           {identityData && (
                               ...
                               >
                                   <div className="absolute top-0 right-0 p-2 opacity-50">
                                       <span className="text-[10px] text-green-300 border border-green-300 px-1">ID_VERIFIED</span>
                                   </div>

                                   <div className="grid grid-cols-2 gap-4 mb-4">
                                       <div>
                                           <p className="text-[10px] text-green-600 uppercase tracking-widest">ALIAS</p>
                                           <p className="text-xl text-green-300 font-bold font-mono">{identityData.alias}</p>
                                       </div>
                                       <div>
                                           <p className="text-[10px] text-green-600 uppercase tracking-widest">CLASS</p>
                                           <p className="text-xl text-green-300 font-bold font-mono">{identityData.class}</p>
                                       </div>
                                       <div>
                                           <p className="text-[10px] text-green-600 uppercase tracking-widest">ORIGIN</p>
                                           <p className="text-sm text-green-300 font-mono">{identityData.origin}</p>
                                       </div>
                                       <div>
                                           <p className="text-[10px] text-green-600 uppercase tracking-widest">SYNAPSE SYNC</p>
                                           <div className="w-full bg-green-900 h-2 mt-1 rounded-full overflow-hidden">
                                               <div className="h-full bg-green-400" style={{ width: identityData.stat }}></div>
                                           </div>
                                       </div>
                                   </div>

                                   <div className="border-t border-green-500/20 pt-4">
                                       <p className="text-[10px] text-green-600 uppercase tracking-widest mb-1">CURRENT MISSION</p>
                                       <ScrambleText text={identityData.mission} className="text-sm text-green-200 font-mono leading-relaxed" />
                                   </div>
                ...
                {/* Corner Decors */}
                <div className="absolute -top-1 -left-1 w-4 h-4 border-l-2 border-t-2" style={{ borderColor: themeColor }}></div>
                <div className="absolute -top-1 -right-1 w-4 h-4 border-r-2 border-t-2" style={{ borderColor: themeColor }}></div>
                <div className="absolute -bottom-1 -left-1 w-4 h-4 border-l-2 border-b-2" style={{ borderColor: themeColor }}></div>
                <div className="absolute -bottom-1 -right-1 w-4 h-4 border-r-2 border-b-2" style={{ borderColor: themeColor }}></div>
